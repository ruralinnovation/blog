...
We can also use `cori.data.fcc` to understand ISP presence in an area. Let's 
begin by pulling data for the state of Vermont.
...
In this example, I'm interested in seeing the footprint for 
[NEK Broadband](https://nekbroadband.org/), a community non-profit 
bringing high-speed broadband to the Northeast Kingdom.

Each ISP has a unique FRN in the Form 477 data. To view, NEK Broadband data. 
we can filter using this unique FRN.
...
Finally, we can use `cori.data.fcc` to see how reported broadband service 
levels vary over time.

To do so, we will use the `get_county_nbm_raw` function to load data for 
Caledonia County, VT from the 2022 and 2023 December NBM releases.
...
The raw county data reports the service levels at each broadband serviceable 
location, ie for one location you can have multiple services. To create block totals, we will need to do some aggregation 
calculations.
...
caledonia_chg <- caledonia_combined %>%
  ...
  mutate(
    good_location = ifelse(has_100_20_service == 1, location_id, NA) 
  ) %>%
  group_by(geoid_bl, year) %>%
  summarise(
    # number of services that has 100/20
    count_100_20_prev = sum(has_100_20_service, na.rm = TRUE),
    # number of locations that has 100/20
    count_100_20 = n_distinct(good_location, na.rm = TRUE),
    # number of location 
  ...
  select(geoid_bl, count_100_20_prev, count_100_20, count_total, n_services, pct_100_20, year)
...
There's a lot more to discover in the package, so I highly recommend you 
check out the [reference documentation](https://ruralinnovation.github.io/cori.data.fcc/reference/index.html).
