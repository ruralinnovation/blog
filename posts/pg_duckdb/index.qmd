...
There are a lot of talks (ightfully) on the use of [Apache Parquet](https://parquet.apache.org/), [Apache Arrow](https://arrow.apache.org/) and [DuckDB](https://duckdb.org/). Let's see an example on how they can be used to convert a table in a PostgresSQL (PG) database to a parquet file.
...
We will use FCC NBM raw data with December 2023's release. You can learn more about it on the FCC [website](https://broadbandmap.fcc.gov/data-download).  

The table we will be converting has a size of 102 GB (without indexes), has 888 176 676 rows and 12 columns and it is the result of importing around 440 CSVs.   

## Getting your credentials

The first step to connect to PG is getting your credentials. We are assuming here that you have a `.pgpass` file located in you home directory (`~/.pgpass`).
...
Deciding how to partition a parquet is both a data and business decision. 
Here `state_abbr` and `technology` are a good tradeoff by having good overall size of parquets file and fast access to regularly needed data.
...
Finally, just for the pleasure let's run some quick query:
...
Impressive isn't it! We will probably dig a bit deeper on those new technologies in more blog posts! 
