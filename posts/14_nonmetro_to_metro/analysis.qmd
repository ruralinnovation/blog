...
chrt_dta <- nonmetro_to_metro_summary %>%
  ...
  # Updated year range: 2018/2019 baseline, 2023/2024 endpoint
  # (originally 2017/2018 → 2022/2023; updated to align with current data vintage)
  filter(year %in% c(2018, 2019, 2023, 2024)) %>%
  ...
  select(category, variable, est_2023, est_2024, tidyr::starts_with("pct")) %>%
  tidyr::pivot_longer(
    cols = c(est_2024, est_2023, tidyr::starts_with("pct"))
  ) %>%
  filter(!is.na(value)) %>%
  filter(
    !(name == "pct_chg_2024" & variable == "pct_emp_in_computer_math_occupations")
  ) %>%
  filter(
    !(name == "pct_chg_2024" & variable == "share_ba_or_higher")
  ) %>%
  filter(!(name == "est_2023" & variable != "Tradable services")) %>%
  mutate(
    variable_label = recode(variable,
    "employment" = "Percent change in employment (2018-2023)",
    "establishments" = "Percent change in businesses (2018-2023)",
    "population" = "Percent change in population (2018-2023)",
    "pct_emp_in_computer_math_occupations" = "Share employed in tech (2024)",
    "share_ba_or_higher" = "Share with a college degree (2024)",
    "broadband_adoption" = "Share of households with a broadband subscription (2024)",
    "median_age" = "Average median age (2024)",
    "Tradable services" = "Share employed in tradable services (2023)"
    ),
...
fig <- chrt_dta %>%
  ...
  labs(
    ...
    subtitle = "By 2024 metropolitan designation",
    x = NULL,
    y = NULL,
