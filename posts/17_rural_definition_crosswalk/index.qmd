---
title: "Crosswalking rural tract definitions - while we wait"
description: "Documenting CORI's rural definition and how we translate the definition to 2020 tract boundaries"
image: visual
author: "Drew Rosebush"
editor: visual
date: "2025-5-2"
categories: [rural definition, crosswalk, census boundaries, spatial data]
---
...
The Center on Rural Innovation is [Mission Statement]. Understanding what it means to be rural is core to this mission. Meaning, that one of the core roles of CORI's data team is to maintain a robust understanding of rural definitions (package). During our work on the Robert Wood Johnson Rural Apperature project - ["Defining rural America"](https://ruralinnovation.us/blog/defining-rural-america/), we identified the variability of how people and government agencies view and understand rural.This work inspired us to generate a custom rural definition that brings together the OMB's county CBSA definition of nonmetro, micropolitan + non-CBSA, alongside ERS's RUCA tract definition. Specifically, CORI classifies any tract (or subgeography) that falls in a nonmetro county OR tract that falls in a metro county that also has a RUCA classification of a 4 or higher, [Micropolitan area core: primary flow within an urban cluster of 10,000 to 49,999 (large UC)](https://www.ers.usda.gov/data-products/rural-urban-commuting-area-codes/documentation), as rural.

With this definition, we're able to capture more detail local characteristics of rural regions across the nation. Tract are our best approximation at census defined  neighborhood boundaries, containing populations with similar housing and socioeconomic characteristics[^neighborhoods].

[^neighborhoods](Geographic Area Reference Manual GARM - https://www2.census.gov/geo/pdfs/reference/GARM/Ch10GARM.pdf)


## 1. What we did

`cori.data.fcc` makes it easy to quickly pull data on broadband service for your area. In particular, using the `get_nbm_bl` function, you can access a CORI-opinionated version of the National Broadband Map's latest release at the Census blockc level for any U.S. county.
